...
function buildCartWhatsAppUrl(items: CartItem[], totalPrice: number): string {
  const bib = items[0]?.bibNumber ?? 0
  const names = items.map((it, i) => {
    const n = it.photo.original_filename
      ? it.photo.original_filename.replace(/\.[^.]+$/, '')
      : it.photo.cloudinary_public_id.split('/').pop() ?? `Foto ${i + 1}`
    return `• ${n}`
  }).join('\n')
  const message = `Hola! Quiero comprar ${items.length} foto${items.length > 1 ? 's' : ''} del corredor *#${bib}*.\n\n${names}\n\nTotal: *$${totalPrice.toLocaleString('es-AR')} ARS*. ¿Me pasás los datos para la transferencia?`
        ...
        {items.length > 0 && (
          <div className="border-t border-slate-100 px-5 pt-4 pb-6 flex flex-col gap-4">
            ...
            <a
              ...
              className="flex items-center justify-center gap-2 bg-[#25D366] hover:bg-[#1fba59] active:scale-[0.98] text-white font-bold text-sm px-5 py-3.5 rounded-2xl transition-all cursor-pointer min-h-[48px]"
            >
